        ...
        match message {
            Ok(msg) => match msg.update_oneof {
                Some(UpdateOneof::Transaction(update)) => {
                    ...
                    if let Some(txn) = update {
                        ...
                        compiled_instructions.iter().for_each(|instruction| {
                            if instruction.instruction.program_id
                                == Pubkey::from_str(PUMPFUN_AMM_PROGRAM_ID)
                                    .expect("Failed to parse public key")
                            {
                                match PumpfunAmmProgramIx::deserialize(&instruction.instruction.data) {
                                    Ok(decoded_ix) => {
                                        let idl: Idl =
                                            serde_json::from_str(&pumpfun_amm_idl).expect("Failed to deserialize IDL");
                                        match idl
                 ...
                 let pump_amm_with_event = pump_amm_formatter(parsed_confirmed_txn_with_meta.clone());
                      if let Some(pump_amm_with_event) = pump_amm_with_event {
                        println!("Pump AMM:\n{:#?}", pump_amm_with_event);
                       }  
                    }
                }
...
pub fn pump_amm_formatter(
    ...
    tx.message.instructions
        ...
        .find(|instr| instr.name.to_lowercase() == "create_pool")?; // returns None if not found
